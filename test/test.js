    ...
    it("#start() stub", function (done) {
      ...
      var stub = sinon.stub(p, "start", function (my_cb) {
        ...
        setInterval(function () {
          switch (++times) {
            case 1:
              my_cb(undefined, new Buffer.from("MyReader\0\0"));
              self.clock.tick(1000);
              break;

            case 2:
              my_cb(undefined, new Buffer.from("MyReader"));
              break;

            case 3:
              my_cb(undefined, new Buffer.from("MyReader1\0MyReader2\0\0"));
              break;
          }
        }, 1000);
        self.clock.tick(1000);
...
describe("Testing CardReader private", function () {
  ...
  describe("#_connect()", function () {
    it("#_connect() success", function (done) {
      var p = get_reader();
      p.on("reader", function (reader) {
        var connect_stub = sinon.stub(
          reader,
          "_connect",
          function (share_mode, protocol, connect_cb) {
    ...
    it("#_disconnect() success", function () {
      ...
      p.on("reader", function (reader) {
        ...
        var connect_stub = sinon.stub(
          reader,
          "_disconnect",
          function (disposition, disconnect_cb) {
